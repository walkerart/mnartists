...
<div class="container">
	<div class="row">
		<?php if(isset($rows['paging'][array_search($rows['reviews']['uid'], $rows['paging'])+1]) && isset($rows['paging'][array_search($rows['reviews']['uid'], $rows['paging'])-1])) : ?>
			<a class="paging pull-right" href="<?php echo url('opportunity/' . $rows['opportunity']['vid'] . '/submission/' . $rows['paging'][array_search($rows['reviews']['uid'], $rows['paging'])+1] . '/admin', array()); ?>">Next<span class="glyphicon glyphicon-chevron-right"></a>
			<a class="paging" href="<?php echo url('opportunity/' . $rows['opportunity']['vid'] . '/submission/' . $rows['paging'][array_search($rows['reviews']['uid'], $rows['paging'])-1] . '/admin', array()); ?>"><span class="glyphicon glyphicon-chevron-left">Previous</a>
		<?php elseif(isset($rows['paging'][array_search($rows['reviews']['uid'], $rows['paging'])+1])) : ?>
			<a class="paging pull-right" href="<?php echo url('opportunity/' . $rows['opportunity']['vid'] . '/submission/' . $rows['paging'][array_search($rows['reviews']['uid'], $rows['paging'])+1] . '/admin', array()); ?>">Next<span class="glyphicon glyphicon-chevron-right"></a>
		<?php elseif(isset($rows['paging'][array_search($rows['reviews']['uid'], $rows['paging'])-1])) : ?>
			<a class="paging" href="<?php echo url('opportunity/' . $rows['opportunity']['vid'] . '/submission/' . $rows['paging'][array_search($rows['reviews']['uid'], $rows['paging'])-1] . '/admin', array()); ?>"><span class="glyphicon glyphicon-chevron-left">Previous</span></a>
		<?php endif; ?>
		<div class="text-center">
			<a href="<?php echo url('opportunity/' . $rows['opportunity']['vid'] . '/submissions'); ?>">
				Application <?php echo array_search($rows['reviews']['uid'], $rows['paging']) + 1 . ' of ' . count($rows['paging']); ?></div>
			</a>
	</div>
	<div class="row">
      	<div class="col-md-8">
      		<h1>
      			<?php echo $rows['reviews']['first_name'] . ' ' . $rows['reviews']['last_name']; ?>
      		</h1>
      		<p><?php echo $rows['reviews']['address']; ?></br>
		...
		<div class="col-md-4">
			<h1><?php echo isset($rows['reviews']['voteCount']) ? $rows['reviews']['voteCount'] . '/' . count($rows['jurors'])  : '0/'. count($rows['jurors']); ?> Reviews Complete</h1>
